...
func TestInsert(t *testing.T) {
	...
	tdb.AddTable("test", map[string]string{"a": "INTEGER", "b": "TEXT"})
	tdb.Insert("test", map[string]string{"a": "1", "b": "testing"})
	tdb.DebugDump("test")
}
...
func TestInsertAgain(t *testing.T) {
	...
	tdb.AddTable("test", map[string]string{"a": "INTEGER", "b": "TEXT"})
	tdb.Insert("test", map[string]string{"a": "2", "b": time.Now().Format(time.RFC3339)})
	tdb.DebugDump("test")
}

func TestWifiTable(t *testing.T) {
	tdb := NewTempDB("wifi", "/tmp")
	...
	tdb.Insert("wifi", map[string]string{
		"event_id":           "42",
...
func TestCleanup(t *testing.T) {
	tdb1 := NewTempDB("test1", "/tmp/test")
	tdb2 := NewTempDB("wifi", "/tmp")
	tdb1.Close()
	tdb2.Close()
	tdb1.Remove()
	tdb2.Remove()
