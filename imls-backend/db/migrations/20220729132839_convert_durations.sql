...
INSERT INTO imlswifi.presences(start_time, end_time, fscs_id, sensor_id, manufacturer_index, patron_index)
    (SELECT to_timestamp(start::int) AS start_time, to_timestamp("end"::int) AS end_time, fcfs_seq_id AS fscs_id, s.sensor_id,
        manufacturer_index AS manufacturer_index, patron_index AS patron_index
     FROM durations_v2 d
