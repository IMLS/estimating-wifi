...
(define (pairer s ls)
  (cond
    [(empty? ls) '()]
    [else
     (cons (list s (first ls))
           (pairer s (rest ls)))]))

(define (make-pairs los)
  (cond
    [(empty? los) '()]
    [else
     (append (pairer (first los) (rest los))
             (make-pairs (rest los)))]))
...
(length common-words)
(define ordered
  (sort common-words (lambda (a b) (< (string-length (~a a))
                                      (string-length (~a b))))))

(define paired (make-pairs (take common-words 1000)))
(length paired)

(with-output-to-file "wordlist.js"
  #:mode 'text
  #:exists 'replace
  (thunk
