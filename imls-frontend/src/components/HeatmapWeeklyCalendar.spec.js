...
describe("HeatmapWeeklyCalendar", () => {
  ...
  it("should render a matrix of values in columns and rows, with percentile shading", () => {
   const wrapper = mount(HeatmapWeeklyCalendar, { props: {
        selectedDate: "1999-12-31", // note: this is on a Friday
        weekStartDateISO:  "1999-12-26",
        dataset: [ [5,4,], [1,2], [2,3], [3,4], [4,5], [5,1], [5,0] ],
    ...
    const allValuesRendered = wrapper.findAll('.weekly-calendar__day .weekly-calendar__cell');
    ...
    expect(wrapper.findAll('.weekly-calendar__day')[5].classes("isSelectedDate")).toBe(true);
    expect(wrapper.findAll('.weekly-calendar__day')[6].classes("isSelectedDate")).toBe(false);
  });
});
